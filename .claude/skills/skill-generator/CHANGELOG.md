...
## V2.0.1 (2026-07-14, same-day patch)

Found while generating the first real `--kind agent` bundle
(`python-engineer`) against ReconForge: `--kind agent` was accepted by the
CLI and produced files in the right place, but the *content* of those
files still hardcoded skill-kind paths in several spots, because
`skill_md.template`, `role_md.template`, and `manifest_json.template` were
written skill-first and reused verbatim for agents without being made
path-aware. Concretely, agent-kind output previously contained:

- `SKILL.md`'s header comment, "Architecture constraints" role reference,
  "Workflow" section, "Validation" section, and "Related assets" section
  all pointed at nonexistent `.claude/{skills,roles,workflows}/<name>/...`
  paths instead of `.claude/agents/<name>/...`, and referenced a
  `workflow.md` / `VALIDATION_CHECKLIST.md` that agent bundles don't
  generate.
- `role.md`'s header comment and "Relationship to other assets" section
  had the same problem.
- `manifest.json`'s `architecture.allowed_write_paths` and `assets` block
  always listed the 5-folder skill layout, even for agent bundles.

**Fix:** `generate()` now computes `self_skill_path`, `role_md_path`,
`manifest_path` (via the existing `manifest_path` local), plus
`workflow_intro_block`, `validation_block`, `related_assets_block`,
`role_relationship_block`, `allowed_write_paths_json`, and `assets_json` —
all branched on `kind` — and passes them into the template context. The
three templates now reference these placeholders instead of hardcoded
skill-shaped paths. Verified via regression test that `--kind skill`
output is byte-for-byte the same shape as before this patch, and that
`--kind agent` output now only ever references files that actually exist
in an agent bundle.

No CLI or file-location behavior changed — this is a content-correctness
